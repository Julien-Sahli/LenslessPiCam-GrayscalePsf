...
from lensless.io import load_3d_data
...
def gradient_descent(
    ...
    print("PSF : " + str(psf.shape))
    print("IMG : " + str(data.shape))
    ...
    for i in range(psf.shape[0]):
        ...
        if method is GradientDescentUpdate.VANILLA:
            recon = GradientDescient(psf[i])
        elif method is GradientDescentUpdate.NESTEROV:
            recon = NesterovGradientDescent(psf[i])
        else:
            recon = FISTA(psf[i])
        recon.set_data(data)
        ...
        if not no_plot:
            plt.show()
        if save:
            np.save(plib.Path(save) / ("final_reconstruction" + ("0" if i<10 else "") + str(i) + ".npy"), res[0])
